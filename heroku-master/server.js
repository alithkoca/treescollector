...
const pool=new pg({host:'ec2-54-75-246-118.eu-west-1.compute.amazonaws.com',
                database:'df8pmnnvo07v5p',
                user:'dacgmwxdvgdcjl',
                password:'00baee341dd86a80a6428bd20c118bacb7edef98d17b900f6d8078e70536a441',
                port:5432,
...
{
    pool.query("SELECT jsonb_build_object('type','FeatureCollection','features', jsonb_agg(feature)) FROM (SELECT jsonb_build_object('type','Feature','geometry',ST_AsGeoJSON(geom)::jsonb, 'properties', to_jsonb(row) - 'gid' - 'geom') AS feature FROM (SELECT * FROM "+table_name+" WHERE tree_type='Palmiye') row) features;", (err1, res1) => {
